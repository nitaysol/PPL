...
export const makeExtEnv = (vs: string[], vals: Value[], env: Env): ExtEnv =>{
    let return_env:Env = ({tag: "ExtEnv",id:generateEnvId(), frame: makeFrame(vs, vals), env: env});
    persistentEnv[return_env.id] = return_env;
    return return_env;
}
...
const makeGlobalEnv = (): GlobalEnv => {
    let return_env:Env = {tag: "GlobalEnv", frame: makeBox(makeFrame([], []))};
    return return_env;
}

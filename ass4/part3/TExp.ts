...
;; Type language
;; <texp>         ::= <atomic-te> | <compound-te> | <tvar>  | <Union-te>
...
export type UnionTExp = {tag:"UnionTExp", types: TExp[]};
export const makeUnionTExp = (types:TExp[]): UnionTExp =>
    ({tag:"UnionTExp", types:types});
...
;; expected exactly one -> in the list or at least one '|'
;; We do not accept (a -> b -> c) - must parenthesize
*/
const parseCompoundTExp = (texps: any[]): ProcTExp | Error | TExp => {
    const posProc = texps.indexOf('->');
    const posUnion = texps.indexOf('|');
    return (posProc != -1) ? parseProcTExp(texps) :
    (posUnion != -1) ? parseUnionTExp(texps) :
    Error('Not a legal compound expression');
};

const parseProcTExp = (texps: any[]): ProcTExp | Error => {
...
const parseUnionTExp = (texps: any[]): TExp | Error => {
    const posUnion = texps.indexOf('|');
    if (posUnion === -1){
        return Error(`Procedure type expression without | - ${texps}`);
    }
    else{
        let toEvaluate = texps.filter((curr) => curr!= '|');
        return safeMakeUnionTExp(toEvaluate.map((curr) => parseTExp(curr)));
    }
};

const safeMakeUnionTExp = (types: Array<TExp|Error>): Error | TExp =>{
    if(hasNoError(types)){
        let toSort = flatUnionTExp(makeUnionTExp(types));
        let sorted = sortUnionTExp(makeUnionTExp(toSort));
        let final = removeDuplicates(makeUnionTExp(sorted));
        if(final.length===1){
            return parseTExp(unparseTExp(final[0]));
        }
        else return makeUnionTExp(final);
    }
    else return Error(getErrorMessages(types));
}

export const flatUnionTExp = (exp:UnionTExp) : TExp[] => {
    let toSort = exp.types.reduce((acc,curr:TExp) => {
        if ((isAtomicTExp(curr)) || (isProcTExp(curr))){
            return acc.concat([curr]);
        }
        else if (isUnionTExp(curr)) {
            return acc.concat(flatUnionTExp(curr))
        }
    }, []);
    return toSort;
}

export const sortUnionTExp = (exp:UnionTExp) : TExp[] => {
    let sorted = exp.types.sort((a: TExp, b: TExp): number => {
        if (isProcTExp(a) && isProcTExp(b))
            return a.returnTE.tag < b.returnTE.tag ? -1 : a.returnTE.tag === b.returnTE.tag ? 0 : +1;
        return a.tag.toLowerCase() < b.tag.toLowerCase() ? -1 : a === b ? 0 : +1; 
    });
    return sorted;
}

export const removeDuplicates = (exp:UnionTExp) : TExp[] => {
    let final = exp.types.reduce((acc:TExp[],curr) => {
        if (acc.length == 0){
            return acc.concat(curr);
        }
        else{
            if (acc[acc.length-1].tag == curr.tag){
                return acc;
            }
            else{
                return acc.concat(curr);
...
export const unparseTExp = (te: TExp | Error): string | Error => {
    ...
    const up = (x: TExp | Error): string | string[] | Error =>
        ...
        isUnionTExp(x) ? x.types.length === 1 ? unparseTExp(x.types[0]) : [map(unparseTExp, x.types).join(' | ')] :

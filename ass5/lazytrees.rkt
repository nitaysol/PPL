...
(define take-lzt
  (lambda (lzt n)
    (if (empty-lzt? lzt)
        '()
        (take-lzt-counter lzt n 0))))

;; Signature:  take-lzt-counter(lzt, n, count)
;; Type: Lzt(T)*Number*Number->List(T)
;; Examples:
;; > (take-lzt-counter (testsmall 0) 8 0)
;; '(0 1 1 2 2 2 2 3)
;; > (take-lzt-counter (testbig 5) 10 0)
;; '(5 6 7 8 7 8 9 8 9 10)
(define take-lzt-counter
  (lambda (lzt n count)
    (let ((nth-list (lzt->nth-level lzt count)))
    (if (or (> n (length nth-list)) (= n (length nth-list)))
            (append nth-list (take-lzt-counter lzt (- n (length nth-list)) (+ count 1)))
            (take nth-list n)))))